...
class DeleteUser200Response(BaseModel):
    ...
    message: StrictStr = Field(description="Status message of the deletion process")
    __properties: ClassVar[List[str]] = ["success", "message"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['User deleted', 'Verification email sent']):
            raise ValueError("must be one of enum values ('User deleted', 'Verification email sent')")
        return value
